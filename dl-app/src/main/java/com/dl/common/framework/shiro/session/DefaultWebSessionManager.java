...
public class DefaultWebSessionManager extends DefaultSessionManager implements WebSessionManager {
	...
	private static Logger log = LoggerFactory.getLogger(DefaultWebSessionManager.class);
	...
	private Serializable getReferencedSessionId(ServletRequest request, ServletResponse response) {
		...
		// 不会把sessionid放在URL后
		/*request.setAttribute(ShiroHttpServletRequest.SESSION_REWRITING_ENABLED, Boolean.FALSE);*/
		return id;
	}
	...
	/**
	 * 把sessionId 放入 response header 中 onStart时调用 没有sessionid时 会产生sessionid 并放入
	 * response header中
	 */
	...
	private void onInvalidation(SessionKey key) {
		...
		if (WebUtils.isHttp(key)) {
			...
			log.debug(
					"SessionKey argument is not HTTP compatible or does not have an HTTP request/response pair. Session ID cookie will not be removed due to invalidated session.");
		}
	...
	protected void onStop(Session session, SessionKey key) {
		...
		if (WebUtils.isHttp(key)) {
			...
			log.debug(
					"SessionKey argument is not HTTP compatible or does not have an HTTP request/response pair. Session ID cookie will not be removed due to stopped session.");
		}
	}
