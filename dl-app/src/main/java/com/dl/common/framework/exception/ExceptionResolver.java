...
public class ExceptionResolver implements HandlerExceptionResolver{
	...
	public ModelAndView resolveException(HttpServletRequest request, HttpServletResponse response, Object obj, Exception ex) {
		if(ex instanceof UnauthorizedException){
	        ...
	        try {
	            response.getWriter().write(ResultUtil.error(ResponseCodeEnum.NO_AUTH));
	        } catch (IOException e) {
	        	logger.error("没有操作权限"+ e.getMessage(), e);
	        }
	        ...
	        response.setHeader("Cache-Control", "no-cache, must-revalidate");
	        try {
	            response.getWriter().write(ResultUtil.error(ResponseCodeEnum.ACCOUNT_EXPIRED));
	        } catch (IOException e) {
	        	logger.error("没有登录认证"+ e.getMessage(), e);
	        }
